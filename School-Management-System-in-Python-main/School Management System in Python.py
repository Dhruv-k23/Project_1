...
connector.execute(
    "CREATE TABLE IF NOT EXISTS USER_AUTH (USERNAME TEXT PRIMARY KEY NOT NULL, PASSWORD TEXT NOT NULL)"
)

def signup():
    username = sd.askstring("Sign Up", "Enter Username:")
    if not username:
        return  # User canceled sign up
    ...
    if existing_user:
        mb.showerror('Sign Up Failed', 'Username already exists. Please choose a different username.')
    else:
        password = sd.askstring("Sign Up", "Enter Password:")
        if password:
            # Add the new user to the database
            query = "INSERT INTO USER_AUTH (USERNAME, PASSWORD) VALUES (?, ?)"
            cursor.execute(query, (username, password))
            connector.commit()
            mb.showinfo('Sign Up Success', 'Sign up successful! You can now log in.')
...
# Function to view teacher record
def view_teacher_record():
    ...
    dob.set_date(date)
    subject_strvar.set(selection[6])
...
Label(teacher_tab, text="TEACHER MANAGEMENT", font=headlabelfont, bg='MediumPurple1').pack(side=TOP, fill=X)
